...
class _ScheduleWidgetState extends State<ScheduleWidget> {
  ...
  Widget build(BuildContext context) {
    final List<String> teamNames = widget.model
        .getTournamentNames(); // gets all the names for the tournaments
    ...
    return Row(children: [
      ...
      ElevatedButton(
          onPressed: () {
            setState(() {
              showPoolPlay =
                  !showPoolPlay; // toggle the view state to show pool play or show bracket play
            });
          },
  ...
  Expanded _poolPlayView() {
    return Expanded(
      ...
      crossAxisAlignment: CrossAxisAlignment
          .stretch, // streches each item to the sides of the screen
      children: [
        Container(
          decoration: BoxDecoration(
            color: Colors.red,
          ), // set the container background color to red
          child: TextButton(
              onPressed: null,
              child: Text('Pool 1',
  ...
  Expanded _listPoolWidgets(List<Game> pool) {
    return Expanded(
      ...
      // creates a list view for the pool
      ...
      itemBuilder: (context, index) {
        ...
        return ListTile(
          title: Row(
            ...
            children: [
              // displays the two teams playing each other
              Text(game.team1),
              Text('VS'),
              Text(game.team2),
            ],
          ...
          subtitle: Text(
              'Court ${game.courtNum} - ${game.time}'), // displays the court number and game time
          onTap: () {
            final game = widget.model
                .fetchGameFromId(1); // gets the game from the game id FIX THIS
            Navigator.push(
                context,
                MaterialPageRoute(
                    builder: (context) => MatchDetails(
                        model: widget.model,
                        info: game))); // goes to match details when clicked
          },
        );
      },
      separatorBuilder: (BuildContext context, int index) => Divider(
          color: Colors.red), // creates a divider between each list tile
    ));
  }
  ...
  SingleChildScrollView _bracketPlayView() {
    ...
    final pool1Teams = teams
        .where((team) => team.rank <= 3)
        .toList(); // filter teams for pool1
  ...
  TBracket<Team> _bracket(List<List<Team>> teams) {
    return TBracket<Team>(
      ...
      onContainerTapDown: (model, tapDownDetails) {
        final match = widget.model
            .fetchMatchesFromId(1); // gets the match from the match id FIX THIS
        Navigator.push(
            context,
            MaterialPageRoute(
                builder: (context) =>
                    MatchDetails(model: widget.model, info: match)));
      }, // goes to match details when clicked
      hadderBuilder: (context, index, count) =>
          Text(''), // no labels above each game
      connectorColor: Colors.red,
      teamContainerDecoration: BracketBoxDecroction(
        // makes the boxes around the names red
        borderRadious: 17,
        color: Colors.red,
      ),
      ...
      teamNameBuilder: (Team t) {
        // creates the individual teams in the bracket
        return BracketText(
          text: t.name,
