...
class CalendarWidget extends State<Calendar> {
  String? _closestTournamentDate;
  DateTime? _selectedDate =
      DateTime.now(); // makes a variable for the current selected date

  @override
  void initState() {
    super.initState();
    _fetchClosestTournamentDate();
  }

  Future<void> _fetchClosestTournamentDate() async {
    final closestDate = await widget.model.fetchClosestTournamentDate();
    setState(() {
      _closestTournamentDate = closestDate;
    });
  }
    ...
    return Column(
      children: [
          ...
          child: TableCalendar(
            ...
            calendarStyle: CalendarStyle(
              todayTextStyle: TextStyle(
                  color: Colors
                      .black), // makes the font color black for today's date so that it shows up
              todayDecoration: BoxDecoration(
                border: Border.all(
                    color: Colors.red), // outlines box for today's date
                shape: BoxShape.circle,
              ),
              selectedDecoration: BoxDecoration(
                // makes the selected date have a red circle
        ...
        if (_selectedDate != null)
          _buildTournamentList(), // displays the tournaments list for the date selected
      ],
    );
  }
      ...
      child: FutureBuilder(
        ...
        builder: (context, snapshot) {
          if (!snapshot.hasData) {
            return Center(
                child:
                    CircularProgressIndicator()); // display loading symbol if still loading
          }
