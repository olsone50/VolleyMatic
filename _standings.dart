...
class StandingsWidget extends State<Standings> {
  ...
  final Map<String, int> _tournamentNameToId =
      {}; // creates a map to store the names and ids of tournaments
  ...
  void initState() {
    ...
    widget.model.fetchTournaments().then((tournaments) {
      // fetch tournaments from the database
      if (tournaments.isNotEmpty) {
        tournaments.sort((a, b) => b['date'].compareTo(
            a['date'])); // sort tournaments by date in descending order

        _selectedName = tournaments
            .first['name']; // gets the name of the most recent tournament
        _selectedId = tournaments.first[
            'tournament_id']; // gets the tournament_id of the most recent tournament
  ...
  Container _dropdownTournamentList() {
    return Container(
      ...
      child: FutureBuilder(
        future: widget.model
            .fetchTournaments(), // get the tournaments from the database
        builder: (context, snapshot) {
          if (snapshot.connectionState == ConnectionState.waiting) {
            ...
            List<String> tournamentNames =
                snapshot.data!.map<String>((tournament) {
                ...
                DropdownButton<String>(
                  ...
                  onChanged: (String? newValue) {
                    setState(() {
                      ...
                      _selectedId =
                          _tournamentNameToId[newValue]!; // get id from the map
                    });
                  },
      ...
      builder: (context, snapshot) {
        if (snapshot.connectionState == ConnectionState.waiting) {
          ...
          for (var teamData in teamsData) {
            ...
            rows.add(
              // adds the name and the wins and losses to the table rows
              TableRow(
                children: [
                  Text(teamName, style: TextStyle(fontSize: 20)),
                  Center(child: Text('$wins', style: TextStyle(fontSize: 20))),
                  Center(
                      child: Text('$losses', style: TextStyle(fontSize: 20))),
                ],
              ),
            );
          }
          return Table(
            // returns a table of the teams in the tournament and their scores
            columnWidths: {
