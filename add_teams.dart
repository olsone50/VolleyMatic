...
class AddTeams extends StatefulWidget {
  const AddTeams({super.key, required this.tournamentList, required this.model});

  final dynamic tournamentList; // gets the tournament information from the add tournament 
...
class _AddTeamsState extends State<AddTeams> {
  ...
  void _addTeam() async {
    await widget.model.addTeam(widget.tournamentList[0], name.text, int.parse(rank.text), _image!.path);
    setState(() {}); // trigger rebuild to update team list after adding the team
  }

  FutureBuilder _teamsList() {
        return FutureBuilder(
          future: widget.model.fetchTeamsFromTournament(widget.tournamentList[0]), // gets the teams using the tournament information
